...
class AssuranceChecksControllerISpec extends BaseISpec {
  ...
  "POST /client-details form" should {
    ...
    "redirect to confirm business when successfully submitting nino" when {
      ...
      "input nino contains only capital letters" in new TestSetupNoJourneyRecord { testInvasiveCheckWithNino("AA123456A") }
      "input nino contains mixed case letters" in new TestSetupNoJourneyRecord { testInvasiveCheckWithNino("Aa123456a") }
      "input nino contains only lowercase letters" in new TestSetupNoJourneyRecord { testInvasiveCheckWithNino("aa123456a") }
      "input nino contains random spaces" in new TestSetupNoJourneyRecord { testInvasiveCheckWithNino("AA1   2 3 4 5 6        A ") }
