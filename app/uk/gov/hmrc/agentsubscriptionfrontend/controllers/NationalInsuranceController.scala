          ...
          agent.authNino match {
            case Some(_) =>
              existingSession.nino match {
                case Some(nino) =>
                  Ok(nationalInsuranceNumberTemplate(ninoForm.fill(nino)))
                case None => Ok(nationalInsuranceNumberTemplate(ninoForm))
              }
    ...
    withSubscribingAgent { implicit agent =>
      withValidSession { (_, existingSession) =>
        ninoForm
          .bindFromRequest()
          .fold(
            formWithErrors => Ok(nationalInsuranceNumberTemplate(formWithErrors)),
            validNino => {
